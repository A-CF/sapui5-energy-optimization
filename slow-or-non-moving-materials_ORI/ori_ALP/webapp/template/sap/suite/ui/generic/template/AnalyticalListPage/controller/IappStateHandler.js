		...
		function resolveAppStatePromise (oAppData, oURLParameters, sNavType) {
			...
			if (sNavType !== sap.ui.generic.app.navigation.service.NavType.initial) {
				...
				if ((!bHasOnlyDefaults || oState.oSmartFilterbar.isCurrentVariantStandard())) {
					...
					if (bHasOnlyDefaults) {
						var oMergedVariant = oTemplateUtils.oCommonUtils.getMergedVariants([new SelectionVariant(JSON.stringify(oState.oSmartFilterbar.getUiState().getSelectionVariant())), oAppData.oSelectionVariant]);
