...
sap.ui.define(
  [
    'ui/s2p/mm/stock/overview/opt3/controller/BaseController',
    'sap/ui/model/json/JSONModel',
    'ui/s2p/mm/stock/overview/opt3/model/formatter',
    'sap/ui/model/Filter',
    'sap/ui/model/FilterOperator',
    'sap/ui/core/format/NumberFormat',
    'sap/m/MessageToast',
    'ui/s2p/mm/stock/overview/opt3/reuse/materialmaster/controller/ValueHelpController',
    'sap/ui/generic/app/navigation/service/NavigationHandler',
    'ui/s2p/mm/stock/overview/opt3/reuse/custom/LightDialog',
    'ui/s2p/mm/stock/overview/opt3/reuse/custom/ChartPersonalizationLight',
    'ui/s2p/mm/stock/overview/opt3/reuse/custom/HistoryChartLight',
    'external/Chartjs'
  ],
  function (B, J, f, F, b, N, M, V, c, LightDialog, ChartPersonalizationLight, HistoryChartLight, Chart) {
    'use strict';
    return B.extend('ui.s2p.mm.stock.overview.opt3.controller.S1', {
      formatter: f,
      onInit: function () {
        var t = this;
        ...
        var p = { container: 'ui.s2p.mm.stock.overview.opt3', item: 'app' };
        ...
        // Throttling table expansion
        /* var plantStorageTable = this.getView().byId("idPlantStorageLocationTable");
        var x = function (event) {
          if (event.getParameter('total') > 0) {
            setTimeout(function () {
              this.expandAllStorageLocations();
            }.bind(this), 100);
            plantStorageTable.detachUpdateFinished(x);
          }
        }.bind(this)
        plantStorageTable.attachUpdateFinished(x); */
      ...
      onCopilot: function (e) {
        ...
        if ((s.length === 1 && s[0].data.Type !== 'total') || e.getParameter('id') === 'idBtnCopilot') {
          ...
          if (!this._oCopilotPopover) {
            this._oCopilotPopover = sap.ui.xmlfragment('ui.s2p.mm.stock.overview.opt3.view.CoPilotChatPopover', this);
      ...
      onDrillUp: function (e) {
        ...
        switch (D) {
          case 'FiscalPeriod':
            break;
          case 'Day':
            var v = this.lightHistoryChart//sap.ui.getCore().byId('newVizFrame');
            var h = v.getModel();
      ...
      _successDayHistoryLoad: function (p, e, s, S, D, r) {
        ...
        var y = sap.ui.getCore().byId('newChartContainer');
        var z = sap.ui.getCore().byId('newDrillDown');
        var A = sap.ui.getCore().byId('newDrillUp');
      ...
      _displayHistoryChart: function (p, s, S) {
        ...
        /** 
         * EDIT: Additional logic to exchange VizFrame with Chart.js chart
        */

        this.lightHistoryChart.setModel(n);
        var res = [];
        var data = n.getProperty('/HistoryData')
        var xValues = data.map((h) => h.PostingDate.toLocaleDateString('en-CA'))
        var categories = Object.keys(data[0]).filter(key => !['PostingDate'].includes(key))
        res = categories.map((cat) => ({
          label: cat,
          data: data.map((h) => h[cat] ?? 0),
          borderColor: this.colorMap[categories.indexOf(cat)],
          fill: false,
          tension: 0.0
        }))
        var html = new sap.ui.core.HTML({
          content: "<canvas id='plotlyHistoryChart' style='width:100%;height:420px'></canvas>",
          afterRendering: function () {
            new Chart('plotlyHistoryChart', {
              type: 'line',
              data: {
                labels: xValues,
                datasets: res
              ...
              options: {
                ...
                scales: {
                  x: {
                    ...
                    title: { display: true, text: this.getResourceBundle().getText('TITLE_DAY_HISTORY_CHART', [o.format(k), o.format(L)]) }
                  },
                  y: {
                    min: 0,
                    grid: {
                    ...
                    title: { display: true, text: this.getResourceBundle().getText('YAXIS_HISTORY_CHART') }
                  }
                },
                elements: {
        ...
        this.lightHistoryChart.plotlyChartContainer.removeAllItems();
        this.lightHistoryChart.plotlyChartContainer.addItem(html);
        this.lightHistoryChart.plotlyChartContainer.rerender();
          ...
          if (r !== '') {
            ...
            if (R !== null) {
              ...
              o.read('/MaterialHeaders', {
                urlParameters: { 
                  $expand: E, 
                  $select: "Material,MaterialName,MaterialType,MaterialTypeName,BaseUnit,BaseUnitName,Material2Plants/Batch,Material2Plants/Plant,Material2Plants/PlantName,Material2Plants/MatPlant2MatPlantStockHistory,Material2Plants/MaterialPlant2MatPlSpecial,Material2Plants/Plant2StorLocs/MatStorLoc2MatStorLocStockHis,Material2Plants/Plant2StorLocs/MatStorLoc2MatStorLocSpecial,Material2Auoms,Material2Plants/CurrentStock,Material2Plants/BlockedStockQuantity,Material2Plants/QualityInspectionStockQuantity,Material2Plants/RestrictedStockQuantity,Material2Plants/ReturnsBlockedStockQuantity,Material2Plants/StockInTransitQuantity,Material2Plants/TiedEmptiesStockQuantity,Material2Plants/TransferStockPlantQuantity,Material2Plants/TransferStockStorageLocQty,Material2Plants/GoodsReceiptBlockedStockQty,Material2Plants/Plant2StorLocs/StorageLocation,Material2Plants/Plant2StorLocs/StorageLocationName,Material2Plants/Plant2StorLocs/CurrentStock,Material2Plants/Plant2StorLocs/BlockedStockQuantity,Material2Plants/Plant2StorLocs/RestrictedStockQuantity,Material2Plants/Plant2StorLocs/ReturnsBlockedStockQuantity,Material2Plants/Plant2StorLocs/TransferStockStorageLocQty,Material2Plants/Plant2StorLocs/QualityInspectionStockQuantity,AlternativeUnit" 
                },
                filters: e,
      ...
      _successMaterialLoad: async function (s, I, D, r) {
        console.log(s, I, D, r)
        var t = this;
        var T = true;
        ...
        //Prepare Chart
        var items = this.getView().getModel('oFrontend').getProperty('/Items')
        var xValues = items.map(d => {
          if (d.StorageLocation == "") {
            return ["", d.Plant]
          } else {
            return [d.StorageLocation, d.Plant]
          }
        });
        var categories = ["BlockedStockQuantity", "CurrentStock", "QualityInspectionStockQuantity", "RestrictedStockQuantity", "ReturnsBlockedStockQuantity", "StockInTransitQuantity", "TiedEmptiesStockQuantity", "TransferStockPlantQuantity", "TransferStockStorageLocQty", "NonVltdGRBlockedStockQty"]
        var res = categories.map(cat => ({
          label: cat,
          backgroundColor: this.colorMap[categories.indexOf(cat)],
          data: items.map(d => d[cat] ?? 0)
        }))
        var html = new sap.ui.core.HTML({
          content: "<canvas id='plotlyChart' style='width:100%;height:420px'></canvas>",
          afterRendering: function () {
            new Chart('plotlyChart', {
              type: 'bar',
              data: {
                labels: xValues,
                datasets: res
              ...
              options: {
                ...
                scales: {
                  x: {
                    ...
                    title: {
                      display: true,
                      text: this.getResourceBundle().getText('REPORTING_DATE_TEXT', [
        ...
        this.getView().byId('barChartPlotly').addItem(html);
        //---
        this._toggleBusy(false);
        ...
        //this.createMicroCharts();
      },
      createMicroCharts: function () {
        var table = this.getView().byId('idPlantStorageLocationTable')
        table.getItems().forEach(function (item, index) {
          var historyData = this.getModel('oFrontend').getProperty(`/Items/${index}`);
          var cells = item.getCells();
          var res = [];
          var currentLabels = []
          Object.entries(historyData.HistoryData).forEach(([stock, dataArr]) => {
            currentLabels = dataArr.map((entry) => `${entry.FiscalPeriod}/${entry.FiscalYear}`)
            var y = dataArr.map((entry) => entry.StockQuantity);
            res.push({
              label: stock,
              data: y,
              borderColor: this.colorMap[index % 10],
              fill: false,
              tension: 0.0,
              borderWidth: 1
            })
          });
          var canvasId = `microChartCustom-${index}`;
          var html = new sap.ui.core.HTML({
            content: `<canvas id='${canvasId}' style='width:100%;height:42px'></canvas>`,
            afterRendering: function () {
              var t = document.getElementById(canvasId);
              new Chart(t.getContext("2d"), {
                ...
                options: {
                  ...
                  plugins: {
                    title: {
                      display: false,
                      label: this.getResourceBundle().getText('STOCK_BY_PLANT_LABEL')
                    },
                    legend: {
                      display: false
                    },
          ...
          cells[cells.length - 1].removeAllItems();
          cells[cells.length - 1].addItem(html);
          cells[cells.length - 1].addEventDelegate({
            onclick: function () {
              var box = cells[cells.length - 1]
              this.handleHistoryChartDisplay({ getSource: () => box })
            }.bind(this)
          })
        }.bind(this));
      },
      ...
      handleSpecialStockLinkPressed: function (e) {
        ...
        if (!this._oPopover) {
          this._oPopover = sap.ui.xmlfragment('ui.s2p.mm.stock.overview.opt3.view.SpecialStockPopover', this);
      ...
      handlePersoButtonPressed: function (e) {
        if (!this._oPersDialog) {
          this._oPersDialog = sap.ui.xmlfragment('ui.s2p.mm.stock.overview.opt3.view.personalization', this);
      ...
      handleColumnSettingsPressed: function (e) {
        if (!this._oColumnSettingsDialog) {
          this._oColumnSettingsDialog = sap.ui.xmlfragment('ui.s2p.mm.stock.overview.opt3.view.column_setting', this);
      ...
      handleHistoryChartDisplay: function (e, I) {
        ...
        if (attachFunc) {
          this.lightHistoryChart.attachReady(function () {
            ...
            var res = [];
            var data = u.getProperty('/HistoryData')
            var xValues = data.map((h) => `${h.FiscalPeriod}/${h.FiscalYear}`)
            var categories = Object.keys(data[0]).filter(key => !['FiscalPeriod', 'FiscalYear'].includes(key))
            res = categories.map((cat) => ({
              label: cat,
              data: data.map((h) => h[cat] ?? 0),
              borderColor: this.colorMap[categories.indexOf(cat)],
            ...
            var html = new sap.ui.core.HTML({
              content: "<canvas id='historyChart' style='width:100%;height:420px'></canvas>",
              afterRendering: function () {
                  ...
                  options: {
                    ...
                    scales: {
                      x: {
                        ...
                        title: { display: true, text: this.getResourceBundle().getText('XAXIS_HISTORY_CHART', [n, q, L, r]) }
                      },
                      y: {
                        min: 0,
                        grid: {
      ...
      handleViewSettingsDialogButtonPressed: function (e) {
        if (!this._oSettingsDialog) {
          this._oSettingsDialog = sap.ui.xmlfragment('ui.s2p.mm.stock.overview.opt3.view.settings', this);
