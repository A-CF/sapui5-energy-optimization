  ...
  return {
    init: function () {
      var u = jQuery.sap.getUriParameters(),
        ...
        e = 'C_SlowOrNonMovingMatlQrySet',
        E = u.get('errorType'),
        i = E === 'badRequest' ? 400 : 500,
        o = jQuery.sap.syncGetJSON(s).data,
        d = o['sap.app'].dataSources,
        b = d.mainService,
        c = jQuery.sap.getModulePath(_ + b.settings.localUri.replace('.xml', ''), '.xml'),
        f = /.*\/$/.test(b.uri) ? b.uri : b.uri + '/',
        A = b.settings.annotations;
      m = new M({ rootUri: f });
      M.config({ autoRespond: true, autoRespondAfter: u.get('serverDelay') || 1000 });
      ...
      A.forEach(function (g) {
        var h = d[g],
          U = h.uri,
          l = jQuery.sap.getModulePath(_ + h.settings.localUri.replace('.xml', ''), '.xml');
        new M({
          rootUri: U,
          requests: [
            {
              ...
              response: function (x) {
                ...
                var k = jQuery.sap.sjax({ url: l, dataType: 'xml' }).data;
                x.respondXML(200, {}, jQuery.sap.serializeXML(k));
                return true;
              },
            },
          ],
