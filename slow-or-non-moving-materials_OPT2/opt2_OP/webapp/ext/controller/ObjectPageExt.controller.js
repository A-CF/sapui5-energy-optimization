...
sap.ui.define([
	...
	"external/Chartjs"
], function (TimeDimension, DateFormat, VBox, Filter, ComparisonMicroChart, ComparisonMicroChartData, SelectionVariant, Chart) {
	"use strict";
	var sControllerName = "ui.s2p.mm.lib.slonomo.mat.opt2.ext.controller.ObjectPageExt";
	sap.ui.controller(sControllerName, {
		onInit: function () {
			...
			/**
			 * Additional logic to create canvas elements for chart.js charts 
			 */
			var stockSituationContainer = this.byId('stockSituationChartContainer');
			stockSituationContainer.setContent('<canvas id=\'stockSituationCustomChart\' style=\'width:100%; max-height:400px;\'></canvas>');
			var stockContainer = this.byId('stockChartContainer');
			stockContainer.setContent('<canvas id=\'stockCustomChart\' style=\'width:100%; max-height:400px;\'></canvas>');
			var timeChartContainer = this.byId('timeChartContainer');
			timeChartContainer.setContent('<canvas id=\'timeCustomChart\' style=\'width:100%; max-height:400px;\'></canvas>');
		},
		 ...
		 * 
		 * EDIT: Alexander Waldner
		 * Enhanced existing functionality to render Chart-js charts
		 * 
		*/
		_successChartDataLoad: function (oData) {
			...
			this.historicalData = aHistoricalData;
		...
		_successPredictModelLoad: function (oData) {
			//var oVizFrame = this.byId("idVizFrame");
			var aHistoricalData = this.historicalData; //oVizFrame.getModel() && oVizFrame.getModel().getData(); // get the historical data from chart
		 ...
		 * 
		 * EDIT: Waldner Alexander
		 * Added grouping logic to retrieve x and y-values,
		 * enhanced functionality to render chart-js charts
		 */
		_loadVizChart: function (aChartType, aChartData, bPredictInactive) {
			var smi_type = aChartType[0] || 'bar';
			var predict_type = aChartType[1] || 'line';

			var x_values = aChartData.map(d => {
				var year = +d.YearPeriod.slice(0, 4);
				var month = +d.YearPeriod.slice(4, 6);
				var date = new Date(Date.UTC(year, month - 1, 1));
				return new Intl.DateTimeFormat('en', { month: 'short', year: "numeric" }).format(date);
			});
			var y_values_smi = aChartData.map(d => d.ConsumptionToStockRatio);
			var y_values_predict = aChartData.map(d => d.PredictRatio);

			if (this.timeCustomChart) this.timeCustomChart.destroy();

			this.timeCustomChart = new Chart(document.getElementById('timeCustomChart'), {
				type: 'bar',
				data: {
					labels: x_values,
					datasets: [
						{
							type: smi_type,
							label: 'Slow-Moving Indicator',
							data: y_values_smi
						},
						{
							type: predict_type,
							label: 'Predicted Slow-Moving Indicator',
							data: y_values_predict
						}
					]
				},
				options: {
					...
					scales: {
						...
						y: {
							title: {
								display: true,
								text: "Slow-Moving Indicator & Predicted Slow-Moving Indicator"
							}
						}
					}
				}
		...
		_loadStockSituationVizChart: function (aChartData) {
			...
			var y_values_consumption = aChartData.map(d => d.CnsmpnQtyInBaseUnitOnRefDate);
			var y_values_quantity = aChartData.map(d => d.MatlWrhsStkQtyInMatlBaseUnit);
			var y_values_bom = aChartData.map(d => d.NumberOfBOMUsingTheMaterial);

			if (this.stockSituationChart) this.stockSituationChart.destroy();

			this.stockSituationChart = new Chart(document.getElementById('stockSituationCustomChart'), {
				type: 'bar',
				data: {
					labels: x_values,
					datasets: [{
						...
						data: y_values_consumption,
						yAxisID: 'stock'
					},
					{
						type: 'line',
						label: 'Stock Quantity',
						data: y_values_quantity,
						yAxisID: 'stock'
					},
					{
						type: 'bar',
						label: 'Active BOM Usage',
						data: y_values_bom,
						yAxisID: 'bom'
					}
					]
				...
				options: {
					...
					scales: {
						stock: {
							...
							beginAtZero: true,
							title: {
								text: "Stock Quantity & Stock Consumption",
								display: true
							}
						},
						bom: {
		...
		_loadSMIPlantVizChart: function (aChartData, sCurrentPlantSMI, sCurrentPlantStockQuantity, sCurrentQuantityUnit) {
			...
			var x_values = aChartData.map(d => `${d.PlantName} (${d.Plant})`);
			var y_values_quantity = aChartData.map(d => d.MatlWrhsStkQtyInMatlBaseUnit);
			var y_values_smi = aChartData.map(d => d.ConsumptionToStockRatio);
			this.stockSituationChart = new Chart(document.getElementById('stockCustomChart'), {
				type: 'bar',
				data: {
					labels: x_values,
					datasets: [
				...
				options: {
					...
					plugins: {
						tooltip: {
							callbacks: {
								afterBody: (items) => {
									var it = items[0].dataIndex;
									var row = aChartData[it];
									return [row['MaterialBaseUnit']]
								}
							}
						},
						legend: {
		...
		_setStockSituationPopoverData: function (oEvent) {
			...
			if (!aSelectedItems[0].data.hasOwnProperty(this._sMeasureStockConsumption)) {
				...
				if (!this._oMovementPopover) {
					this._oMovementPopover = sap.ui.xmlfragment("ui.s2p.mm.lib.slonomo.mat.opt2.ext.fragment.MovementPopover", this);
...
/* 
  ...
  - ext/fragment/SMIPlantChart: removed vizframe and added HTML element to host the chart-js
  canvas
  - ext/fragment/StockSituationTimeSeriesChart: removed vizframe and added HTML element to host the chart-js
  canvas
  - ext/fragment/TimeSeriesChart: removed vizframe and added HTML element to host the chart-js
  canvas
*/