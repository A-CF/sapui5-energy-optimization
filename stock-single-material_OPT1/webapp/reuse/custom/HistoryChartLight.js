...
], function (Control, ChartContainer, ChartContainerContent, Popover, VizFrame, OverflowToolbar, Button, ToolbarSpacer, ChartContainerToolbarPlaceholder, Text, VBox) {
    "use strict";
    return Control.extend("ui.s2p.mm.stock.overview.opt1.reuse.custom.HistoryChartLight", {
        metadata: {
            properties: {
        ...
        */
        init: function () {
            this._i18n = new sap.ui.model.resource.ResourceModel({
                bundleUrl: jQuery.sap.getModulePath('ui.s2p.mm.stock.overview.opt1') + '/i18n/i18n.properties',
            });
        },
        /** 
        ...
        */
        initContent: function () {
            this.popOver = new Popover({
                id: "newChartContainerPopover"
            })
            this.chart = new VizFrame({
                id: "newVizFrame",
                width: "100%",
                uiConfig: { applicationSet: 'fiori' },
                selectData: () => this.getCaller().onVizFrameSelectData(),
                deselectData: () => this.getCaller().onVizFrameDeselectData()
            })
            this.popOver.connect(this.chart.getVizUid());
